...
export class Member implements IMember {
	...
	store: IMemberStore = MemberStore.getInstance();
	...
	addMemberToWorkspace(
		...
		console.log(
			`Adding member with ID: ${member.id} to workspace ID: ${workspaceId}`
		);
	}
	...
	getMember(id: TMemberId): IMemberDTO | null {
		...
		console.log(
			`Fetching member with ID: ${id} from workspace ID: ${this.member.id}`
		);
		return null;
	}
	...
	updateMember(id: TMemberId, member: Partial<IMemberDTO>): void {
		...
		console.log(
			`Updating member with ID: ${id} in workspace ID: ${this.member.id}`
		);
	...
	removeMember(id: TMemberId): void {
		...
		console.log(
			`Removing member with ID: ${id} from workspace ID: ${this.member.id}`
		);
	}
	...
	get listMembers(): IMemberDTO[] {
		...
		console.log(
			`Fetching list of members from workspace ID: ${this.member.id}`
		);
		return [];
	}
	...
	removeFromWorkspace(
		...
		console.log(
			`Removing member with ID: ${memberId} from workspace ID: ${workspaceId}`
		);
	}
